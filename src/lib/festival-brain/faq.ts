...
export const FAQ: Record<FaqKey, FaqEntry> = {
  ...
  vendor_part_payment: {
    ...
    fact:
      'We always encourage a vendor to settle their full stall fee as soon as they can, ideally in full by the end of September 2026, with 15 October 2026 the firm fallback date for this round, because paying sooner helps the organisers put on the best possible show (locking in the marquee, power and marketing). We push for one to two payments. If a single payment is not possible, we can set up a payment plan of two instalments (a third only as an absolute last resort), with the final instalment by 15 October 2026, or 1 November, then 15 November, and 30 November 2026 at the very latest. They propose the exact date and amount of each instalment, it is approved automatically a few minutes later, and instalments are paid by EFT with the proof sent to support@youngatheart.co.za (a card checkout takes the full amount at once). Until a plan is approved or the fee is settled, the current due date of the vendor still stands and they still receive the payment reminder emails.',
