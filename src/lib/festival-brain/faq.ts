...
export const FAQ: Record<FaqKey, FaqEntry> = {
  ...
  vendor_part_payment: {
    ...
    fact:
      'We always encourage a vendor to settle their full stall fee by the end of September 2026 first. If a single payment this month is not possible, we can set up a payment plan: two instalments by default (a third on request), with the final instalment by the end of October 2026, or mid-November, and 30 November 2026 at the very latest. They propose the exact date and amount of each instalment, it is approved automatically a few minutes later, and instalments are paid by EFT with the proof sent to support@youngatheart.co.za (a card checkout takes the full amount at once). Until a plan is approved or the fee is settled, the current due date of the vendor still stands and they still receive the payment reminder emails.',
