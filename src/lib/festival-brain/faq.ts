...
export const FAQ: Record<FaqKey, FaqEntry> = {
  ...
  vendor_part_payment: {
    ...
    fact:
      'Vendors can pay their stall fee in instalments through a payment plan. They propose the exact date and amount of each instalment (2 to 6 of them, every date on or before 30 November 2026, together covering the full outstanding fee), and it is approved automatically a few minutes later. A single card checkout takes the full amount at once, so instalments are paid by EFT with the proof sent to support@youngatheart.co.za. A vendor who would rather make one full payment with more time can take until 31 August 2026 to settle in full instead. Until a plan is approved or the fee is settled, the current due date of the vendor still stands and they still receive the payment reminder emails.',
