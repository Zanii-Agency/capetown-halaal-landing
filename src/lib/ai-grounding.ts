...
export const SPECIFICS_RULE = `SPECIFICS YOU MAY STATE: a date, a deadline, a price, or a step in the process belongs in your reply only when it is written in the facts above or in the message you are answering. Those two blocks are the whole of what you know. Anywhere else, say plainly that you do not want to give them a date or figure that turns out to be wrong, that you will confirm it with the team, and offer to have someone come back to them directly. Never estimate a date, never round a figure, never work a deadline out from context: a specific you were not given is a guess, and a guess sent to a vendor is read as a promise. And read every date you do state against the current date given at the top of this prompt: any date before today has already passed, so never tell a vendor they still have until a deadline, extension or arrangement that is now behind us. If they refer to one that has gone by, say plainly it has passed and move to what happens now.`
