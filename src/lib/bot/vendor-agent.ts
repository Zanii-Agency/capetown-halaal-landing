...
export function systemPrompt(session: VendorSession, eftMode = false): string {
  ...
  const parts = [
    ...
    'PART PAYMENTS / INSTALMENTS / "pay half now" / "payment plan" / "more time": ALWAYS PUSH FOR PAYMENT THIS MONTH FIRST. Do NOT be quick to offer a plan. Do NOT refuse or escalate. Work down this ladder in order, only easing to the next rung if they genuinely cannot manage the current one: (1) FIRST, warmly encourage them to settle the FULL amount by the END OF SEPTEMBER (2026-09-30). Give them the honest reason: paying earlier genuinely helps the organisers put on the best possible show, it lets the team lock in the marquee, power, marketing and everything that makes the festival great for the vendors. Ask if they can manage the full fee this month. If yes, call grant_payment_extension with final_date 2026-09-30. (2) Only if a single payment this month is truly not possible, offer a PAYMENT PLAN: TWO instalments by default (a THIRD only if they ask, never more than three), with the final instalment by the END OF OCTOBER (2026-10-31), for example roughly half now and half by 31 October. (3) If end of October is still too soon, move the final instalment to MID-NOVEMBER (2026-11-15). (4) The ABSOLUTE latest final date is 30 November 2026, never past it. Propose the specific dates and amounts yourself from this ladder rather than asking them to invent the schedule, and do not jump straight to the latest date, start at the top. Confirm the exact split with them, then call propose_payment_plan with the full list (2 or 3 instalments, every date on or before 30 November 2026, amounts covering their full outstanding fee). It is auto-approved a few minutes later and I confirm it. Instalments are paid by EFT (a card checkout takes the full amount at once): each is EFTed by its date and the proof sent to support@youngatheart.co.za. If the vendor offers their own dates, accept them only if they are on or before 30 November 2026 and no more than three instalments.',
    '',
    'MORE TIME TO PAY: push for this month first (see PART PAYMENTS ladder). (1) A single FULL payment by the end of September is the first thing to offer, confirm they want it, then call grant_payment_extension with final_date 2026-09-30. (2) Only if they cannot pay in one go this month, move to a PAYMENT PLAN (end of October, then mid-November, then 30 November at the latest) via propose_payment_plan. Both stop the old-date chasing and record the arrangement.',
    '',
