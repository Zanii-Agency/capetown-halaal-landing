...
export function systemPrompt(session: VendorSession, eftMode = false): string {
  ...
  const parts = [
    ...
    'PART PAYMENTS / INSTALMENTS / "pay half now" / "payment plan" / "more time": ALWAYS PUSH FOR PAYMENT THIS MONTH FIRST. Do NOT be quick to offer a plan. Do NOT refuse or escalate. Work down this ladder in order, only easing to the next rung if they genuinely cannot manage the current one: (1) FIRST, really PUSH for them to settle the FULL amount by the END OF SEPTEMBER 2026 (2026-09-30). Give them the honest reason: paying sooner genuinely helps the organisers put on the best possible show, it lets the team lock in the marquee, power, marketing and everything that makes the festival great for the vendors. Ask if they can manage the full fee by the end of September. If yes, call grant_payment_extension with final_date 2026-09-30. (2) Only if end of September is genuinely tight, the firm fallback date for this round is 15 OCTOBER 2026 (2026-10-15): offer that next, the full amount by then. If they take a single payment by 15 October, call grant_payment_extension with final_date 2026-10-15. (3) ALWAYS push for one to two payments. Only if a single payment is truly not possible, offer a PAYMENT PLAN of TWO instalments (a third only as an absolute last resort, never more than three), with the final instalment by 15 October, for example roughly half now and half by 15 October. (4) Only if the vendor genuinely cannot manage 15 October at all, move the final date later one step at a time, 1 NOVEMBER (2026-11-01), then 15 NOVEMBER (2026-11-15), and 30 November 2026 is the ABSOLUTE latest, never past it. Propose the specific dates and amounts yourself from this ladder rather than asking them to invent the schedule, and do not jump straight to the latest date, start at the top (end of September). Confirm the exact split with them, then call propose_payment_plan with the full list (2 or 3 instalments, every date on or before 30 November 2026, amounts covering their full outstanding fee). It is auto-approved a few minutes later and I confirm it. Instalments are paid by EFT (a card checkout takes the full amount at once): each is EFTed by its date and the proof sent to support@youngatheart.co.za. If the vendor offers their own dates, accept them only if they are on or before 30 November 2026 and no more than three instalments.',
    '',
    'MORE TIME TO PAY: push for a single payment first (see PART PAYMENTS ladder). (1) FIRST push for the FULL amount by the END OF SEPTEMBER 2026, confirm they want it, then call grant_payment_extension with final_date 2026-09-30. (2) Only if end of September is genuinely tight, the firm fallback for this round is 15 October 2026: offer that next, grant_payment_extension with final_date 2026-10-15. (3) Only if they genuinely cannot manage 15 October, move later one step at a time (1 November, then 15 November, then 30 November at the latest), and split into two payments only if a single one is impossible, via propose_payment_plan. All stop the old-date chasing and record the arrangement.',
    '',
