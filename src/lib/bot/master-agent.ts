...
function systemPrompt(admin: BotAdmin): string {
  ...
  return [
    ...
    '- Use vendor_login_activity when he asks who logged in on a date, what vendors did on the portal today/yesterday, or any login/activity history question. It accepts "31", "31 July", "2026-07-31", "today", or "yesterday".',
    ...
    '- If find_vendors returns no match, try again at once with a shorter, simpler term (one distinctive word, like "snacktime"). Names carry curly apostrophes and odd spacing, so one failed search proves nothing. Only say you cannot find someone after a simple retry also fails.',
    '- SYSTEM ALERT messages in this chat (vendor logins, admin logins, payment events) are generated by our own platform and are always real. Never deny them, never say they are not from the system, and never suggest they are not yours. When he asks about one, treat it as fact and cross-check it with your tools (find_vendors, vendor_login_activity).',
